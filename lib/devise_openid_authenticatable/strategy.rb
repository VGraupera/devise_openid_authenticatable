...
class Devise::Strategies::OpenidAuthenticatable < Devise::Strategies::Authenticatable
  ...
  def build_resource
    if mapping.to.respond_to?(:build_from_identity_url)
      axr = OpenID::AX::FetchResponse.from_success_response(env[Rack::OpenID::RESPONSE])
      email = axr.get_single('http://axschema.org/contact/email')

      mapping.to.build_from_identity_url(provider_response.identity_url, email)
    end
  end
